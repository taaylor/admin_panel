...
def _get_ids_and_state_table(
        cursor: psycopg.Cursor, table: Literal['person', 'genre'], \
            state: State) -> tuple[list[UUID], datetime.datetime]:
    query = f''' 
        SELECT id, modified 
        FROM content.{table}
        WHERE modified > %s
        ...
        LIMIT 100;
    '''
    last_state = state.get_state(config.PERSON_STATE) \
        if table == 'person' else state.get_state(config.GENRE_STATE)
...
def _get_updated_person_and_genre(connect: psycopg.Connection, state: State) \
      -> tuple[tuple, tuple]:
...
def _get_updated_film_work(connect: psycopg.Connection, state: State) \
    ...
    with closing(connect.cursor()) as psql_cursor:
        ...
        states = {
            config.FW_STATE: last_state_fw, config.GENRE_STATE: genre[1],
